...
keys_to_include_in_hash = ['product_name', 'domain', 'url']
...
def scroll_over_all_docs():
    data = es.search(index="product", scroll='1m',  body={"query": {"match_all": {}}})
    # Get the scroll ID
    sid = data['_scroll_id']
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
      if len(array_of_ids) > 1:
        ...
        matching_docs = es.mget(index="product", doc_type="doc", body={"ids": array_of_ids})
        es.delete(index="product", id=array_of_ids[0])
